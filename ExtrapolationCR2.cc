...
void AlphaMethod::ExtrapolationCR2(){
  ...
  if(AlphaMethod::syst==0 || AlphaMethod::syst==1 || AlphaMethod::syst==2){
    F_CR_TTbar = new TFile(AlphaMethod::CR_path+AlphaMethod::TTFAKEfile,"READ");
  ...
  if(AlphaMethod::syst==3){
    F_CR_TTbar = new TFile(AlphaMethod::CR_path+AlphaMethod::_systematic+AlphaMethod::TTFAKEfile,"READ");
  ...
  for(int i=0; i<n_catnew; i++){
    ...
    SR_TTbar_Int[i]=(TH1D*)F_SR_TTbar_RealTaus->Get(catsnew[i]+"/N"); // needed for the uncertainty on the integral
    CR_TTbar_Int[i]=(TH1D*)F_CR_TTbar_RealTaus->Get(catsnew[i]+"/N"); // needed for the uncertainty on the integral
  }
  ...
  for(int i=0; i<n_catnew; i++){
    CR_Allbkg[i]=(TH1D*)CR_TTbarReal[i]->Clone();
    CR_Allbkg[i]->Add(CR_TTbar[i]);
    ...
    for(int j=1; j<n_bins+1; j++){
      ...
      CR_TTbar[i]->SetBinContent(j, CR_TTbar[i]->GetBinContent(j) * dataTTbar[i]->GetBinContent(j) / CR_Allbkg[i]->GetBinContent(j));
    }
  }
  ...
  if(AlphaMethod::syst==0 || AlphaMethod::syst==3){
    for(int i=0; i<n_catnew; i++){
  ...
  for(int i=0; i<n_catnew; i++){
    CR_TTbar[i]->Scale(SR_TTbar_Int[i]->Integral()/CR_TTbar[i]->Integral());
  }
  cout << "before adding the real tau part" << endl;
  for(int i=0; i<n_catnew; i++){
    cout << "#ttbar from data, " << catsnew[i] << ": " << dataTTbar[i]->Integral() << endl;
    cout << "#ttbar(SR), " << catsnew[i] << ": " << SR_TTbar_Int[i]->Integral() << endl;
  }
  ...
  for(int i=0; i<n_catnew; i++){
    ...
    dataTTbar[i]->Draw("E1");
    SR_TTbar_Int[i]->SetLineColor(1);
    SR_TTbar_Int[i]->SetLineWidth(2);
    SR_TTbar_Int[i]->SetLineStyle(1);
    SR_TTbar_Int[i]->Draw("E1 SAME");
    ...
    leg2->AddEntry(SR_TTbar_Int[i], "t#bar{t} MC (SR)");
